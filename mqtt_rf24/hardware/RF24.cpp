...
void RF24::begin(void)
{
  ...
  write_register(SETUP_RETR, ARD_1250uS | ARC_15); //the comment above contradicts the code!
...
void RF24::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  csn(LOW);
  spi_transfer( W_ACK_PAYLOAD | ( pipe & PIPE_MASK ) );
  const uint8_t max_payload_size = 32;
...
{
  if ( enable )
    write_register(EN_AA, ENAA_ALL);
  else
    write_register(EN_AA, ENAA_DISABLE);
}
